...
# helpers
# ******************************************
def _filter_group_min1(dfg, col, topn=0):
    """
    ...
    if topn==1:
        ...
        return dfg[dfg[col].isin(np.sort(dfg[col].unique()))]
    
    else:
        return dfg[dfg[col].isin(np.sort(dfg[col].unique())[:topn])]


from d6tjoin.utils import _applyFunMulticore, _set_values

class MergeTop5Diff(object):
    """
    ...
    def __init__(self, df1, df2, fuzzy_left_on, fuzzy_right_on, fun_diff=None, exact_left_on=None, exact_right_on=None,
                 top_limit=None, topn=0, fun_preapply = None, fun_postapply = None, is_keep_debug=False, use_multicore=True):
    ...
    def _allpairs_candidates(self):
        ...
        if self.cfg_topn>1 or self.cfg_topn==0:
    ...
    def _top1_diff_noblock(self):
        ...
        df_diff = df_candidates.groupby('__top1left__',group_keys=False).apply(lambda x: _filter_group_min1(x,'__top1diff__'))
    ...
    def _top1_diff_withblock(self):
        ...
        df_diff = df_diff.groupby(self.cfg_exact_left_on+['__top1left__'],group_keys=False).apply(lambda x: _filter_group_min1(x,'__top1diff__'))
        if self.cfg_top_limit is not None:
            df_diff = df_diff[df_diff['__top1diff__']<=self.cfg_top_limit]
    ...
    def merge(self):
        ...
        for ilevel, ikey in enumerate(self.cfg_fuzzy_left_on):
            ...
            if self.cfg_fun_diff[ilevel]:
                df_diff_bylevel[ikey] = MergeTop5Diff(self.dfjoined, self.dfs[1], keyleft, keyright, self.cfg_fun_diff[ilevel], cfg_exact_left_on, cfg_exact_right_on, top_limit=self.cfg_top_limit[ilevel], use_multicore=self.cfg_use_multicore).top1_diff()[0]
            else:
                if typeleft == 'int64' or typeleft == 'float64' or typeleft == 'datetime64[ns]':
                    ...
                    df_diff_bylevel[ikey] = MergeTop5Diff(self.dfjoined, self.dfs[1], keyleft, keyright, jellyfish.levenshtein_distance, cfg_exact_left_on, cfg_exact_right_on, top_limit=self.cfg_top_limit[ilevel], use_multicore=self.cfg_use_multicore).top1_diff()[0]
        ...
        return {'merged': self.dfjoined, 'top5': df_diff_bylevel, 'duplicates': None}
